...
* 调用对象的 hashCode：偏向锁的对象 MarkWord 中存储的是线程 id，调用 hashCode 导致偏向锁被撤销`因为偏向锁没有把 MarkWord 存到线程的锁记录里面, 不能恢复 hashcode`
...
类似于 Monitor 中的 waitset, 已经抢占到锁的线程调用了 wait() 方法后,`将当前拥有该锁的线程就进入 waitset 里面进行等待,进入 waiting 状态`,当其他线程调用了 notifyAll() 方法后会被唤醒去有机会抢占锁. 而条件变量 Condition 也有相似的功能,但是它不止一个,可以创建多个条件变量 Condition condition = lock.newCondition(); 在获得锁的线程内调用一个 condition 的 await() 方法会`将当前拥有该锁的线程放入当前condition`



synchronized 中也有条件变量，就是我们讲原理时那个 waitSet 休息室，当条件不满足时进入 waitSet 等待ReentrantLock 的条件变量比 synchronized 强大之处在于，它是支持多个条件变量的，这就好比synchronized 是那些不满足条件的线程都在一间休息室等消息而 ReentrantLock 支持多间休息室，有专门的等烟的休息室、专门的等早餐的休息室、唤醒时也是按休息室来唤醒
...
- await的线程被唤醒(或打断、或超时)会重新竞争lock 锁
...
CopyOnWriteArraySet 底层对 CopyOnWriteArrayList 进行了 包装，装饰器模式
...
[CompletableFuture使用详解_sermonlizhi的博客-CSDN博客](https://blog.csdn.net/sermonlizhi/article/details/123356877?ops_request_misc=%7B%22request%5Fid%22%3A%22166885040216800184148877%22%2C%22scm%22%3A%2220140713.130102334..%22%7D&request_id=166885040216800184148877&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_click~default-2-123356877-null-null.142^v65^control,201^v3^control_2,213^v2^t3_control2&utm_term=Completablefuture&spm=1018.2226.3001.4187)
...
线程A可能会抢占到cpu资源,这时她会直接从休眠的语句后面开始执行,不会进行条件判断,这就成为虚假唤醒

解决方案: 不使用 if 判断,使用 while 循环判断



#### 集合线程安全问题

ArrayList 是线程不安全的,解决方法有:

1. 使用 Vector (古老)
2. 使用 Collections.synchronizedList(list); (古老)
3. `使用 CopyOnWriteArrayList() 写时复制`

HashSet 有同样的问题

`可以使用 CopyOnWriteArraySet();`
